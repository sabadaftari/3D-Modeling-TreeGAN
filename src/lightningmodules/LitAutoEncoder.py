  ...
  def validation_step(self, valbatch, valbatch_idx):
      if valbatch_idx<=10:
        ...
        if valbatch_idx==0:
          ...
          self.chamfer_loss = torch.concat([self.chamfer_loss.unsqueeze(0), chamfer_loss.unsqueeze(0)], dim=0)
          self.generated_samples = torch.cat([self.generated_samples[0],generated_samples[0]], dim=0)
